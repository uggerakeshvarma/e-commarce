...
import './Add_mobile.css';  // Import the CSS file

export function Add_mobile() {

    const { register, handleSubmit } = useForm();
    const [mobileadd, setMobileadd] = useState([]);

    async function savemobile(data) {
        ...
        formData.append('Mobile_image', mobileadd);

        const config = {
            headers: { "content-type": "multipart/form-data" }
        };
        await axios.post('http://localhost:3001/savemobile', formData, config);
        window.location.reload();
    }

    function Loadmobile(event) {
        var reader = new FileReader();
        reader.onload = function () {
            setMobileadd(reader.result);
        }
    ...
    return (
        <>
            <form onSubmit={handleSubmit((data) => { savemobile(data) })}>
                <h2>ADD Mobile</h2>
                ...
                <input type="file" placeholder="Mobile_image" {...register('Mobile_image')} onChange={(event) => { Loadmobile(event) }} /><br />
